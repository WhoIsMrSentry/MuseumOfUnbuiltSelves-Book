...
// @ts-ignore  virtual module from plugins/mdx-mtime.ts
...
export async function enrichBookMeta(book: Book): Promise<Book> {
  ...
  // Prefer a dedicated book-level asset named "cover.*" if present in assets/
  const coverCandidates = ['cover.png', 'cover.jpg', 'cover.jpeg', 'cover.webp', 'cover.avif', 'cover.svg'];
  let resolvedCover = '';
  for (const c of coverCandidates) {
    const key = `${STORIES_PREFIX}${book.bookSlug}/assets/${c}`;
    if (assetFiles[key]) {
      resolvedCover = assetFiles[key];
      break;
    }
  }
  // Fall back to first-page frontmatter `cover` if no dedicated cover asset exists
  if (!resolvedCover) resolvedCover = resolveAsset(book.bookSlug, meta.cover || '');
